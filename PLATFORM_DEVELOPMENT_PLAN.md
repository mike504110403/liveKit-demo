...
#### ⚠️ 遊戲服務獨立啟動
- **遊戲服務需求確認**（Week 1-2）：與外部遊戲團隊確認需求與 API 規格
...
##### 後端開發（6 個服務完全並行）⚡⚡⚡
1. **認證服務**（2 週）⚡ 由 1 名工程師負責
   - 註冊/登錄
   - JWT 簽發與驗證
   - 第三方登錄（微信、Apple）
   - 簡化權限管理

2. **用戶服務**（2 週）⚡ 由 1 名工程師負責
   - 用戶資料管理
   - 個人主頁
   - 關注/粉絲系統（簡化版）

3. **房間服務**（3 週）⚡ 由 1 名工程師負責
   - 房間創建/銷毀
   - 房間列表/搜索
   - 觀眾管理

4. **直播服務**（3 週）⚡ 由 1-2 名工程師負責
   - 推流地址生成
   - 拉流地址生成
   - 直播狀態管理
   - WebSocket 聊天室
...
5. **支付服務**（3 週）⚡ 由 1 名工程師負責
   - 充值流程
   - 支付寶/微信支付對接（使用 SDK）
   - 訂單管理
   - 錢包系統

6. **遊戲中介層**（3 週）⚡ 由 1 名工程師負責
   - 遊戲指令接收與轉發（從直播服務）
   - 外部遊戲服務 API 對接
   - 遊戲結算結果處理
...
| 角色 | 人數 | 職責 | 招聘/現有 | 優先級 |
|------|------|------|----------|--------|
| **技術長** | 1 | 整體架構設計、直播技術研究、團隊管理 | ✅ 現有 | - |
| **後端 Tech Lead** | 1 | 微服務架構、技術選型、代碼審查 | 🔍 待招聘 | P0 |
| **後端工程師（Go）** | 6-7 | 業務邏輯開發、接口實現（每人負責 1 個服務，含遊戲中介層） | 🔍 待招聘 | P0-P1 |
| **遊戲中介層負責工程師** | 1 | **💜 遊戲服務獨立主理**（Week 1-26，專職負責） | 🔍 待招聘 | P0 |
| **iOS Team Lead** | 1 | iOS 架構設計、代碼審查 | 🔍 待招聘 | P0 |
| **iOS 工程師** | 2 | iOS 原生開發 | 🔍 待招聘 | P1 |
| **Android Team Lead** | 1 | Android 架構設計、代碼審查 | 🔍 待招聘 | P0 |
| **Android 工程師** | 2 | Android 原生開發 | 🔍 待招聘 | P1 |
| **DevOps 工程師** | 1 | CI/CD、環境部署、監控 | 🔍 待招聘或外包 | P1 |
| **QA 工程師** | 2-3 | 測試計畫、自動化測試、執行測試 | 🔍 待招聘或外包 | P2 |
| **UI/UX 設計師** | 1-2 | 原型圖、UI 設計 | 🔍 外包 | P0 |
| **產品經理** | 1 | 需求管理、優先級排序 | ✅ 現有 | - |
| **專案經理** | 1 | 協調、進度追蹤、風險管理 | 🔍 待招聘或兼任 | P1 |

**總人數**: 19-22 人（較原計畫增加 50%）

**備註**: 遊戲服務由外部團隊提供，不計入內部團隊人數

### 資源需求（6 個月壓縮版）

#### 人力成本（月）
- **後端團隊**: 6-7 人 × $5,000-8,000 = $30,000-56,000
- **Mobile 團隊**: 6 人 × $5,000-8,000 = $30,000-48,000
- **DevOps/QA**: 3-4 人 × $4,000-6,000 = $12,000-24,000
- **UI/UX**: 1-2 人 × $3,000-5,000 = $3,000-10,000（外包）
- **專案管理**: 1 人 × $5,000-7,000 = $5,000-7,000
- **月成本估算**: $80,000-145,000（取決於經驗與地區）
- **6 個月總成本**: $480,000-870,000
